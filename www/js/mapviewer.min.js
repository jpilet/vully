...
CanvasTilesRenderer.prototype.getLocation=function(){var left=this.pinchZoom.worldPosFromViewerPos({x:0,y:this.canvas.height/2});var right=this.pinchZoom.worldPosFromViewerPos({x:this.canvas.width,y:this.canvas.height/2});return{x:(left.x+right.x)/2,y:(left.y+right.y)/2,scale:Utils.distance(right,left)};};CanvasTilesRenderer.prototype.setLocation=function(location){var canvas=this.canvas;var ratio=[('vx'in location?location['vx']:.5),('vy'in location?location['vy']:.5)];var x_pos=canvas.width*ratio[0];var y_pos=canvas.height*ratio[1];var constraints=[{viewer:{x:x_pos-canvas.width/2,y:y_pos},world:{x:location.x-location.scale/2,y:location.y}},{viewer:{x:x_pos+canvas.width/2,y:y_pos},world:{x:location.x+location.scale/2,y:location.y}},];this.pinchZoom.processConstraints(constraints);};CanvasTilesRenderer.prototype.refresh=function(){var t=this;if(t.lastRefreshRequest==t.numDraw){return;}
