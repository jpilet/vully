...
if(bottomright.y<this.element.height){var center=(T[5]==0?.5:1);T[5]+=center*(this.element.height-bottomright.y);}};PinchZoom.prototype.checkAndApplyTransform=function(newTransform){newTransform=newTransform||this.transform;this.enforceConstraints(newTransform);if(this.transform!==newTransform){this.transform=newTransform;if(this.transformChanged){this.transformChanged(this.transform);}}};function CanvasTilesRenderer(params){this.params=(params!=undefined?params:{});this.canvas=params.canvas;if(!("tileSize"in this.params))this.params.tileSize=256;if(!("url"in this.params)){this.params.url=function(scale,x,y){return"http://a.tile.openstreetmap.org/"+scale+'/'+x+'/'+y+'.png';};}
...
this.inDraw=true;this.numDraw=0;this.numCachedTiles=0;this.disableResize=false;this.lastRefreshRequest=-1;var t=this;this.pinchZoom=new PinchZoom(t.canvas,function(){if(t.params.onLocationChange){t.params.onLocationChange(t);}
t.location=t.getLocation();if(t.params.debug){t.debug('location: w:'+t.canvas.width
+' h:'+t.canvas.height
+' x:'+t.location.x+' y:'+t.location.y
+' s:'+t.location.scale);}
t.refresh();},this.params.width,this.params.height);this.pinchZoom.minScale=this.params.minScale;this.inDraw=false;this.location=params.initialLocation||{x:this.params.width/2,y:this.params.height/2,scale:this.params.width};this.setLocation(this.location);}
CanvasTilesRenderer.prototype.getLocation=function(){var left=this.pinchZoom.worldPosFromViewerPos({x:0,y:this.canvas.height/2});var right=this.pinchZoom.worldPosFromViewerPos({x:this.canvas.width,y:this.canvas.height/2});return{x:(left.x+right.x)/2,y:(left.y+right.y)/2,scale:Utils.distance(right,left)};};CanvasTilesRenderer.prototype.setLocation=function(location){var canvas=this.canvas;var constraints=[{viewer:{x:0,y:canvas.height/2},world:{x:location.x-location.scale/2,y:location.y}},{viewer:{x:canvas.width,y:canvas.height/2},world:{x:location.x+location.scale/2,y:location.y}},];this.pinchZoom.processConstraints(constraints);};CanvasTilesRenderer.prototype.refresh=function(){var t=this;if(t.lastRefreshRequest==t.numDraw){return;}
